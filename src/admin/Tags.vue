<template>
  <div class="app-container">
    <div class="filter-container clearfix">
      <div class="filter">
        <el-input v-model="newTag" style="width: 200px;" placeholder="新标签名称"></el-input>
        <el-button class="add-btn" @click="handleCreate" type="primary" icon="el-icon-edit">创建新标签</el-button>
      </div>
    </div>
    <el-table :data="tags" border fit highlight-current-row >
...
<script>
  import {getTags, addTags, patchTag, delTag} from "../api/api";
  ...
  export default {
    ...
    methods: {
      ...
      handleCreate() {
        if (this.newTag) {
          addTags({name: this.newTag}).then(res => {
            this.$notify({
              title: '成功',
              message: '添加成功',
              type: 'success'
